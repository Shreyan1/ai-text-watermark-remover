...
| "was **never** trained on..." vs "was trained on..." | n/a | ✅ rejected |
| "**All** 12 regions reported" vs "**None** of the 12" | n/a | ✅ rejected |
...
| 2 | 0.462 -> 0.462 | 1.00 | n/a | ❌ no, every candidate failed a gate |
...
## Stage 0: file provenance on real chat downloads

`tests/test_metadata.py`. The same essay prompt was given to three assistants and
each answer downloaded normally. None of the three mentions its origin in the
prose. All three carry it in the filesystem, where macOS shows it in Finder's Get
Info panel under "Where from":

| source | `kMDItemWhereFroms` | quarantine | filename |
|---|---|---|---|
| ChatGPT | `https://chatgpt.com/` | `0081;...;Arc;5F667D4F-...` | vendor-named |
| Gemini | `https://contribution.usercontent.google.com/download?c=...` | `0081;...;Arc;95B2B9DF-...` | vendor-named |
| Grok | `https://grok.com/` | `0081;...;Arc;786F8BC7-...` | vendor-named |

Three traces each, none visible in an editor, none touched by rewriting a word.

Decoding the Gemini `c=` parameter (base64) yields `bard_storage`,
`response_data`, and the 48-hex-character id
`e28349d995dad45a00065927d1449194037055f63804c310`. That is a per-response
identifier: it ties the file not just to a vendor but to one specific
generation. It is the strongest link of the three and invisible unless you look.

**Two false-positive lessons, both from measurement:**

- `com.apple.provenance` was originally flagged as a leak. It is not: a file
  created locally that was never downloaded carries it too
  (`echo hello > new.txt`), it holds no source URL, and macOS re-creates it on
  write. Reporting it ended every run with a "NOT removable" warning, including
  for clean files. A warning that always fires trains people to ignore warnings.
- `os.getxattr` and its siblings are documented "Availability: Linux" and **do
  not exist on macOS**, the one platform where `kMDItemWhereFroms` matters. The
  first version caught the resulting `AttributeError` and reported "no metadata
  found" for a file that demonstrably carried a chatgpt.com URL. macOS now goes
  through `/usr/bin/xattr`, and a platform with neither path raises rather than
  quietly reporting a clean bill of health.

**Scorer on the same three files**, unmodified as downloaded:
...
None reached the 0.70 human threshold and none was confidently called AI, which
is the honest outcome for a ranking heuristic on five-paragraph essays. Two of
the three used em-dashes, the habit that makes `em_dash_rate` a feature at all.
...
- **Not proven here:** removal of *Google's* watermark (their key is unmeasurable, the real-world constraint); a production-grade regenerator (needs a stronger
