...
yet exist, is defeated the moment its output passes through the pipeline, because we removed the ground it was standing on without ever looking at it.
...
## 5. The pipeline: stages ⓪ to ⑤

```
       ┌───────────────────────── convergence loop (I2) ──────────────────────┐
       │                                                                       │
FILE ─▶ ⓪ Meta ─▶ ① Scrub ─▶ ② Extract ─▶ ③ Regenerate ─▶ ④ Score ─▶ ⑤ Gate ─┬─▶ OUT
       (xattrs)    (code)      (LLM)        (LOCAL LLM)     (stats)           │
                                                 ▲          └─ fail ─▶ back to ③
                                            VoiceProfile       (more aggressive)
```

**⓪ Meta, `scrub/metadata_scrubber.py`** *(pure code, operates on a PATH)*
Provenance that is not in the prose at all, and therefore survives every later
stage untouched. Stage ③ replaces every token, so no statistical watermark
survives it, but a file saved from a chat UI still carries its origin in the
filesystem. Measured on three real downloads (ChatGPT, Gemini, Grok), each
carried three traces: `kMDItemWhereFroms` holding the source URL, a
`com.apple.quarantine` stamp naming the downloading browser, and a filename
beginning with the vendor's name. The Gemini URL additionally embeds a unique
per-response identifier, which links the file not merely to a vendor but to one
specific generation.

This stage is separate from ① because it takes a path rather than a `Document`.
By the time text is a `Document` it has lost the file it came from, and that
file is exactly where this metadata lives. It also inverts the usual order of
trust: `scrub_file` re-inspects the file afterwards and reports what actually
survived rather than trusting its own delete calls, because writing the file can
re-create attributes that were just removed.
...
[arXiv:2507.12782](https://arxiv.org/abs/2507.12782)), so no floor can fix it, it needs a second check that looks at what embeddings discard. `FactChecker` is
...
classifier to retrain as models drift. The one feature no tool can synthesise, *the line only the author could have written*, is explicitly out of scope and
